...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const { firebaseAuthTokenLogin, mustSetUsername, setMustSetUsername } = useAuth();
  ...
  const handleSocialLogin = async (token: string) => {
    try {
      await firebaseAuthTokenLogin(token);
      if (!mustSetUsername) {
        onClose();
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <UsernameSetupModal isOpen={mustSetUsername} onClose={handleUsernameSetupComplete} />
    </>
  );
};
